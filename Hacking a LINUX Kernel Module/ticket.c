...
MODULE_AUTHOR("Eugene A. Shatokhin / Batul Zamin");
MODULE_LICENSE("GPL");
	    ...
	    loff_t *f_pos)
{
  ...
  ssize_t retval = 0;
  int ticket = dev->ticket_number;
	
  if (mutex_lock_killable(&dev->ticket_mutex))
    return -EINTR;
	
  // your code starts here
  ticket += 1000; // initializes first ticket number to 1000
  if (count == 4) {
  	if (copy_to_user(buf, &(ticket), count))
  		return -EFAULT;
  	dev->ticket_number++; // increment ticket number by 1
  	retval = count;
  }
  else {
  	retval = -EINVAL;
